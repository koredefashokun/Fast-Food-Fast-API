...
import { testNoEmptyString } from '../../../helpers/validation';
...
router.post('/', adminMiddleware, async (req, res) => {
  ...
  } else if (!description || typeof description !== 'string' || !testNoEmptyString(description)) {
    res.status(400).json({
      success: false,
      message: 'Please enter the meal description correctly! (Hint: description must be a string)'
    });
  } else if (!imageUrl || typeof imageUrl !== 'string' || !testNoEmptyString(imageUrl)) {
    res.status(400).json({
      success: false,
